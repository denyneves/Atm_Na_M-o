...
const initSidebar = () => {
    ...
    const avatars = ["img/modelo.jpg", "img/modelo.jpg", "img/modelo.jpg", "img/modelo.jpg"]; // Exemplo de caminhos
...
/* ==================================================
   JAVASCRIPT - PÁGINA SOBRE NÓS
   (Colar dentro do DOMContentLoaded ou no final de script.js)
   ================================================== */

// --- Dados da Equipa ---
// Certifique-se que os caminhos das imagens e links estão corretos!
const teamData = [
    { id: 'Al', name: 'Antonio Lunguela', role: 'CEO & Fundador', imgPath: 'img/modelo.jpg', bio: 'Engenheiro de Software visionário e líder nato, Antonio fundou a Pixelhub com a missão de impulsionar a transformação digital em Angola. Combina expertise técnica com uma forte visão de negócio e paixão por criar soluções de impacto.', portfolioDesc: 'Idealizador e arquiteto estratégico do ATM na Mão. Liderou múltiplos projetos de software B2B e B2C no mercado angolano, focando em usabilidade e escalabilidade.', portfolioLink: 'https://www.linkedin.com/in/antonio-longuela-/', socials: { linkedin: 'https://www.linkedin.com/in/antonio-longuela-/' } },
    { id: 'Ac', name: 'Avelino Candundo', role: 'Diretor Administrativo', imgPath: 'img/sravelino.jpeg', bio: 'Especialista em Relações Internacionais e Expert em Administração de negocios, o Sr Avelino garante a eficiência operacional e administrativa da Pixelhub. Sua experiência em negociação e desenvolvimento de parcerias é crucial para o crescimento sustentável da empresa.', portfolioDesc: 'Responsabilidades do diretor administrativo: implementar e monitorar políticas e procedimentos administrativos, gestão de recursos humanos, processos e parcerias estratégicas da Pixelhub e do projeto ATM na Mão.', portfolioLink: '#', socials: { linkedin: '#' } }, // Adicionar link LinkedIn se existir
    { id: 'An', name: 'Augusto Neves', role: 'Engenheiro Informático', imgPath: 'img/augusto.jpg', bio: 'Mestre em Ciência da Computação com profundo conhecimento em arquiteturas cloud-native, segurança e inteligência artificial. Augusto lidera a equipa técnica, garantindo a excelência, inovação e robustez das nossas plataformas.', portfolioDesc: 'Arquiteto técnico principal do ATM na Mão, responsável pela infraestrutura backend, APIs e implementação de tecnologias emergentes.', portfolioLink: 'https://github.com/devAugustoNeves/', socials: { linkedin: '#', github: 'https://github.com/devAugustoNeves/' } }, // Adicionar link LinkedIn se existir
    { id: 'Bm', name: 'Belmiro Miguel', role: 'Lead Mobile Developer', imgPath: 'img/belmiro.jpg', bio: 'Desenvolvedor mobile apaixonado e experiente em React Native, Swift e Kotlin. Belmiro foca em performance nativa, código limpo e experiências de utilizador fluidas, liderando o desenvolvimento das nossas aplicações móveis.', portfolioDesc: 'Líder técnico no desenvolvimento do aplicativo ATM na Mão, focando na otimização de performance e implementação de funcionalidades complexas.', portfolioLink: 'https://github.com/Belmiro-Miguel/', socials: { linkedin: '#', github: 'https://github.com/Belmiro-Miguel/' } }, // Adicionar link LinkedIn se existir
    { id: 'Gs', name: 'Jeovanny Gabriel', role: 'Designer - Diretor de Arte', imgPath: 'img/geovane.jpg', bio: 'Designer talentoso com um olhar apurado para estética e usabilidade. Geovani traduz ideias complexas em interfaces intuitivas e visualmente apelativas, garantindo que a interação com nossos produtos seja sempre agradável e eficiente.', portfolioDesc: 'Criador do design system e responsável pela identidade visual e experiência do utilizador do ATM na Mão e outros produtos Pixelhub.', portfolioLink: '#', socials: { linkedin: '#', dribbble: '#' } }, // Adicionar links reais
    { id: 'Em', name: 'Ezequiel Manuel', role: 'Engenheiro Informático', imgPath: 'img/ezequiel.png', bio: 'Engenheiro de backend pragmático e eficiente, com expertise em Node.js, Python e gestão de bases de dados distribuídas. Ezequiel é fundamental na construção e manutenção da espinha dorsal dos nossos sistemas.', portfolioDesc: 'Desenvolveu e otimizou módulos críticos da API do ATM na Mão, focando em segurança, baixa latência e alta disponibilidade dos serviços.', portfolioLink: '#', socials: { linkedin: '#', github: '#' } }, // Adicionar links reais
    { id: 'Js', name: 'Jose Antonio', role: 'Diretor de Estrategías de Marketing', imgPath: 'img/jose.jpg', bio: 'Comunicador estratégico e Estrategista de Conteúdos, José constrói e nutre a comunidade Pixelhub/ATM na Mão. É a ponte entre a empresa e os utilizadores, gerindo a presença online e campanhas de comunicação.', portfolioDesc: 'Responsável pelas estratégias de marketing, gestão de redes sociais e engajamento da comunidade para o ATM na Mão.', portfolioLink: '#', socials: { linkedin: '#', twitter: '#' } } // Adicionar links reais
];

// --- Populando Grid da Equipa ---
...
if (teamListContainer) {
    teamData.forEach(member => {
        ...
        card.setAttribute('data-reveal', 'fade-in-up');
        const imagePath = member.imgPath || 'img/placeholder_member.png'; // Usa placeholder se imgPath estiver vazio
        card.innerHTML = `
            <img src="${imagePath}" alt="${member.name}" class="team-member-card__img" loading="lazy" onerror="this.onerror=null;this.src='img/placeholder_member.png';">
    ...
    // Adiciona um console.warn se estiver na página sobre.html mas o container não for encontrado
    if (window.location.pathname.includes('sobre.html')) {
         console.warn("Container da lista da equipa (#team-list-container) não encontrado na página 'sobre'.");
    }
}

// --- Lógica do Modal da Equipa ---
const modalOverlay = document.getElementById('team-member-modal');
const modalContent = modalOverlay?.querySelector('.team-modal-content');
const modalCloseButton = modalOverlay?.querySelector('.team-modal__close');
...
const modalSocialsContainer = document.getElementById('modal-member-socials');
const modalPortfolioTitle = document.getElementById('portfolio-title');
const modalPortfolioDesc = document.getElementById('modal-member-portfolio-desc');
const modalPortfolioLink = document.getElementById('modal-portfolio-link');

// Adiciona listeners APENAS se o container da equipa existir (evita erros em outras páginas)
if (teamListContainer && modalOverlay && modalContent && modalCloseButton && modalImg && modalName && modalRole && modalBio && modalSocialsContainer && modalPortfolioTitle && modalPortfolioDesc && modalPortfolioLink) {
    teamListContainer.addEventListener('click', (e) => {
       const triggerCard = e.target.closest('.team-member-card');
       if (!triggerCard) return;
       const memberId = triggerCard.getAttribute('data-member-id');
       const member = teamData.find(m => m.id === memberId);

       if (member) {
           modalImg.src = member.imgPath || 'img/placeholder_member.png';
           modalImg.onerror = () => { modalImg.src = 'img/placeholder_member.png'; };
           modalImg.alt = member.name;
           modalName.textContent = member.name;
           modalRole.textContent = member.role;
           modalBio.textContent = member.bio || 'Mais detalhes sobre este membro em breve.';

           const hasPortfolioDesc = member.portfolioDesc && member.portfolioDesc.trim() !== '';
           const hasPortfolioLink = member.portfolioLink && member.portfolioLink !== '#';

           if (hasPortfolioDesc || hasPortfolioLink) {
               modalPortfolioTitle.style.display = 'inline-block';
               modalPortfolioDesc.textContent = hasPortfolioDesc ? member.portfolioDesc : '';
               modalPortfolioDesc.style.display = hasPortfolioDesc ? 'block' : 'none';
               modalPortfolioLink.href = hasPortfolioLink ? member.portfolioLink : '#';
               modalPortfolioLink.style.display = hasPortfolioLink ? 'inline-flex' : 'none';
           } else {
               modalPortfolioTitle.style.display = 'none';
               modalPortfolioDesc.style.display = 'none';
               modalPortfolioLink.style.display = 'none';
           }
               ...
               Object.entries(member.socials).forEach(([platform, url]) => {
                   if(url && url !== '#') {
                       ...
                       switch (platform.toLowerCase()) {
                           case 'linkedin': iconClass = 'ri-linkedin-box-fill'; break;
                           case 'twitter': iconClass = 'ri-twitter-x-fill'; break;
                           case 'github': iconClass = 'ri-github-fill'; break;
                           case 'dribbble': iconClass = 'ri-dribbble-fill'; break;
                           case 'facebook': iconClass = 'ri-facebook-box-fill'; break;
                           case 'instagram': iconClass = 'ri-instagram-fill'; break;
                       }
                       modalSocialsContainer.innerHTML += `
                           <a href="${url}" target="_blank" rel="noopener noreferrer" aria-label="${member.name} on ${platform}"><i class="${iconClass}"></i></a>
                       `;
                   }
               });
           }
           ...
           document.body.classList.add('modal-open'); // Adiciona classe para travar scroll
           modalContent.scrollTop = 0;
       }
   });

   // Função de fechar modal (pode ser global se usada por outros modais)
   function closeModal() {
       if(modalOverlay) modalOverlay.classList.remove('active');
        // Remove a classe do body APENAS se nenhum outro modal estiver ativo
        // (Se tiver outros modais, precisará de uma lógica mais robusta aqui)
        if (!document.querySelector('.modal-overlay.active')) { // Exemplo simples
   ...
   document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && modalOverlay?.classList.contains('active')) closeModal(); });
     ...
     if (window.location.pathname.includes('sobre.html')) { // Avisa só se estiver na página
          console.warn("Elementos essenciais para o modal da equipa não foram encontrados.");
     }
}


// --- Inicialização Slick Carousel ---
 try { // Usar try/catch para o caso de jQuery ou Slick não carregarem
    if (typeof $ !== 'undefined' && $.fn.slick) {
        const slickElement = $('.workspace-carousel');
        if (slickElement.length) { // Verifica se o elemento existe na página
            slickElement.slick({
                dots: true, infinite: true, speed: 600,
                slidesToShow: 3, slidesToScroll: 1, autoplay: true, autoplaySpeed: 4000,
                arrows: false, centerMode: true, centerPadding: '20px',
                focusOnSelect: true, lazyLoad: 'ondemand', pauseOnHover: true,
                responsive: [
                    { breakpoint: 992, settings: { slidesToShow: 2, centerPadding: '15px' } },
                    { breakpoint: 576, settings: { slidesToShow: 1, centerPadding: '30px' } }
                ]
            });
        } else {
             if (window.location.pathname.includes('sobre.html')) {
                  console.log("Elemento .workspace-carousel não encontrado para inicialização do Slick.");
             }
        }
    } else {
        if (window.location.pathname.includes('sobre.html')) {
             console.warn("jQuery ou Slick Carousel não carregado.");
        }
    }
 } catch(e) {
      console.error("Erro ao inicializar Slick Carousel:", e);
 }


// --- Inicialização ScrollReveal ---
 try { // Usar try/catch para o caso de ScrollReveal não carregar
    if (typeof ScrollReveal !== 'undefined') {
        const srConfig = {
            origin: 'bottom', distance: '50px', duration: 1600, delay: 100,
            easing: 'cubic-bezier(0.5, 0, 0, 1)', scale: 0.95, reset: false, viewFactor: 0.2
        };
       ScrollReveal(srConfig).reveal('[data-reveal]', { interval: 80 }); // Aplica a todos data-reveal
       ScrollReveal({ ...srConfig, origin: 'top', distance: '60px' }).reveal('.hero-sobre .sobre-section__title, .hero-sobre .hero-logo');
       ScrollReveal({ ...srConfig, delay: 150 }).reveal('.hero-sobre .sobre-section__subtitle');
       ScrollReveal({ ...srConfig, interval: 100, scale: 1, distance: '40px' }).reveal('.info-card');
       ScrollReveal({ ...srConfig, duration: 1800, scale: 1 }).reveal('.carousel-card-wrapper');
       ScrollReveal({ ...srConfig, interval: 60, distance: '30px' }).reveal('.team-member-card');
       ScrollReveal({ ...srConfig, origin: 'left', distance: '60px' }).reveal('.contact-details');
       ScrollReveal({ ...srConfig, origin: 'right', distance: '60px' }).reveal('.contact-map');
    } else {
        if (window.location.pathname.includes('sobre.html')) {
             console.warn("ScrollReveal não carregado.");
        }
    }
 } catch(e) {
...
// --- Smooth scroll para links internos (se não for global) ---
 ...
 // --- Atualizar ano no footer (já está no início do DOMContentLoaded) ---

// Fim do código específico da página Sobre
